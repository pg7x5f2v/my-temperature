...
def temp_list(request):
    ...
    return render(request, 'temperature/temp_list.html', {'temps': temps})

def temp_detail(request, pk):
    temp = get_object_or_404(Temp, pk=pk)
    return render(request, 'temperature/temp_detail.html',{'temp': temp})
...
def temp_new(request):
    if request.method == "POST":
        ...
        form = TempForm()
    return render(request, 'temperature/temp_edit.html', {'form':form})
